...
Oui, Astra Trident peut être installé hors ligne. Voir link:../trident-install/kubernetes-deploy.html["Découvrez l'installation d'Astra Trident"^].
...
L'installateur Trident crée un ClusterRole Kubernetes, qui dispose d'un accès spécifique aux ressources PersistentVolume, PersistentVolumeClaim, StorageClass et Secret du cluster Kubernetes. Reportez-vous à link:../trident-install/kubernetes-customize-deploy-tridentctl.html["Personnalisez l'installation tridentctl"^].



=== Est-il possible de générer localement les fichiers de manifeste exacts qu'Astra Trident utilise pour l'installation ?

Vous pouvez générer et modifier localement les fichiers manifestes exacts utilisés par Astra Trident pour l'installation, si nécessaire. Consultez link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personnalisez l'installation tridentctl"^].
