...
Pour effectuer un test, demandez l'accès à « Easily Deploy and Clone Persistent Storage for Containerized Workloads » link:https://www.netapp.com/test-drives/["Test Drive NetApp"^] en utilisant une image de laboratoire prête à l'emploi. Le test vous offre un environnement sandbox avec un cluster Kubernetes à trois nœuds et Astra Trident installés et configurés. C'est un excellent moyen de vous familiariser avec Astra Trident et d'explorer ses fonctionnalités.
