...
ASTRA Trident fournit le `solidfire-san` pilote de stockage pour communiquer avec le cluster. Les modes d'accès pris en charge sont : _ReadWriteOnce_ (RWO), _ReadOnlyMey_ (ROX), _ReadWriteMaly_ (RWX), _ReadWriteOncePod_ (RWOP).

Le `solidfire-san` pilote de stockage prend en charge les modes _file_ et _block_ volume. Pour le `Filesystem` volumeMode, Astra Trident crée un volume et un système de fichiers. Le type de système de fichiers est spécifié par la classe de stockage.
