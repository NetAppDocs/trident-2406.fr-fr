...
Si vous ajoutez un cluster à Astra Control Service sans qu’Astra Trident ne soit préalablement installé, le cluster sera marqué comme  `Eligible`. Après avoir ajouté le cluster à Astra Control, Astra Control Provisioner sera automatiquement activé.
...
. (Registres personnalisés uniquement) Suivez ces étapes pour déplacer l'image vers votre registre personnalisé. Si vous n'utilisez pas de registre, suivez les étapes de l'opérateur Trident dans le <<first-step,section suivante>>.
+
...
. [[première étape]]Activez Astra Control Provisioner dans Astra Trident en utilisant la méthode d'installation que vous avez utilisée à l'origine :
+
[role="tabbed-block"]
====
