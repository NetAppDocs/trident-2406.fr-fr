...
ASTRA Trident fournit le `gcp-cvs` pilote pour communiquer avec le cluster. Les modes d'accès pris en charge sont : _ReadWriteOnce_ (RWO), _ReadOnlyMey_ (ROX), _ReadWriteMaly_ (RWX), _ReadWriteOncePod_ (RWOP).
...
* *CVS-Performance* : le type de service Astra Trident par défaut. Ce type de service optimisé pour les performances convient le mieux aux charges de travail de production qui privilégient la performance. Le type de service CVS-Performance est une option matérielle prenant en charge les volumes d'une taille minimale de 100 Gio. Vous pouvez choisir l'une des options suivanteslink:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["trois niveaux de service"^] :
+
** `standard`
** `premium`
** `extreme`
...
| `storageClass` | Paramètre facultatif utilisé pour spécifier le type de service CVS. Utilisez `software` pour sélectionner le type de service CVS. Sinon, Astra Trident suppose le type de service CVS-Performance (`hardware`). |  
