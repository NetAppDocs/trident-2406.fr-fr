...
.Avant de commencer
* Assurez-vous d'avoir activé Astra Control Provisioner sur le cluster Red Hat OpenShift géré. Consultez link:../trident-use/enable-acp.html["Activez le mécanisme de provisionnement Astra Control"^] pour obtenir des instructions.
* Vérifiez que vous avez accès à l' `tridentctl` utilitaire.
